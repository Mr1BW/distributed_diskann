...
public:
            ...
            std::vector<char> &outbox_buf, BSP::IterationStatus &status)
    {
        ...
        if(superstep%2 == 0){//偶数次superstep，第一个parallel for，即 greedySearch那部分
            comm_time.start();
            size_t recv_counts = inbox_buf_len / ((BP.R + 1) * sizeof(indexType));
            parlay::parallel_for(0, recv_counts, [&](size_t i) {
                const indexType* block_start = reinterpret_cast<const indexType*>(inbox_buf + i * (BP.R + 1) * sizeof(indexType));
        ...
        if (count >= m)
        {
            ...
            comm_time.start();
            //对齐，R个邻居+自身索引
            outbox_buf.resize((BP.R + 1) * sizeof(indexType) * new_out_.size());
            parlay::parallel_for(0, new_out_.size(), [&](size_t i) {
                size_t actual_neighbors_count = new_out_[i].size();
                std::memcpy(outbox_buf.data() + i * (BP.R + 1) * sizeof(indexType), new_out_[i].data(),
                    actual_neighbors_count * sizeof(indexType));
                //填充-1
                if(actual_neighbors_count < BP.R) {
                    std::memset(outbox_buf.data() + i * (BP.R + 1) * sizeof(indexType) + actual_neighbors_count * sizeof(indexType), 0xff,
                        (BP.R - actual_neighbors_count) * sizeof(indexType));
                }
                // 将节点索引存储在最后一个位置
                std::memcpy(outbox_buf.data() + i * (BP.R + 1) * sizeof(indexType) + BP.R * sizeof(indexType), &new_node_id[i], sizeof(indexType));
            });
            ...
            outbox_buf.resize((BP.R + 1) * sizeof(indexType) * grouped_by.size());
            parlay::parallel_for(0, grouped_by.size(), [&](size_t i) {
                size_t actual_neighbors_count = G[grouped_by[i].first].size();
                std::memcpy(outbox_buf.data() + i * (BP.R + 1) * sizeof(indexType), G[grouped_by[i].first].begin(),
                    actual_neighbors_count * sizeof(indexType));
                //填充-1
                if(actual_neighbors_count < BP.R) {
                    std::memset(outbox_buf.data() + i * (BP.R + 1) * sizeof(indexType) + actual_neighbors_count * sizeof(indexType), 0xff,
                ...
                // 将节点索引存储在最后一个位置
                std::memcpy(outbox_buf.data() + i * (BP.R + 1) * sizeof(indexType) + BP.R * sizeof(indexType), &grouped_by[i].first, sizeof(indexType));
            });
            comm_time.stop();
            inc += 1;
